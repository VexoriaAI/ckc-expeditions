...
// UPDATE: (Passo 7.1 - Callback de Combate)
// - handleCombatExecution agora processa o loot real do inimigo.
// - Integração completa com CombatSystem e ModalManager.
// ==================================================================== */
...
import { STATIC_MAP_DATA, MAP_BIOMES, SPAWN_NODE_ID, MAP_NODES } from '../../database/maps.js';
...
// --- Helpers ---
const getKidDataById = (kidId) => { return MOCK_KIDZ_NFTS.find(kid => kid.id === kidId); };
const getSpawnNodeId = (kidTribe) => { return SPAWN_NODE_ID; };
const getCurrentBiomeKey = (nodeId) => { 
    const node = MAP_NODES.find(n => n.id === nodeId);
    return node ? node.biome : 'WASTELAND'; 
};
const rollDice = (min, max) => { return Math.floor(Math.random() * (max - min + 1)) + min; };
const calculateMPCost = (fromNodeId, toNodeId) => { return 1; };
...
export const ExpeditionManager = {
    ...
    moveToNode: function(targetNodeId) {
        ...
        if (!targetNode || !currentNode || !currentNode.connections.includes(targetNodeId)) {
    ...
    /**
     * (ATUALIZADO) Helper para executar a simulação de combate.
     * Agora processa o loot real do inimigo (materials, components, etc.).
     */
    handleCombatExecution: function(enemyData) {
        const state = getState();
        let { expedition } = state;

        // 1. Executa a Simulação Pura
        const combatResult = CombatSystem.simulateCombat(expedition.kidStats, enemyData);

        // 2. Atualiza o HP do Jogador
        expedition.currentHP = combatResult.playerRemainingHP;

        // 3. Processa Vitória ou Derrota
        let lootItems = [];
        
        if (combatResult.victory) {
            expedition.log.unshift(`VICTORY against ${enemyData.name}!`);
            
            // Rola o Loot do Inimigo (Rewards)
            if (enemyData.rewards) {
                for (const [itemId, dropRule] of Object.entries(enemyData.rewards)) {
                    if (rollDice(1, 100) <= dropRule.chance) {
                        ...
                        if (qty > 0) {
                            // (Lógica de detecção de tipo de item)
                            // Por enquanto, assumimos que tudo no 'rewards' do inimigo são materiais ou componentes
                            if (itemId.startsWith('mat_')) {
                                expedition.foundLoot.materials[itemId] = (expedition.foundLoot.materials[itemId] || 0) + qty;
                            } else if (itemId.startsWith('comp_') || itemId.endsWith('_core')) {
                                // Componentes não são stackáveis no inventário principal, mas aqui simplificamos para o log
                                // No 'endExpedition', eles serão convertidos em objetos
                                for(let i=0; i<qty; i++) {
                                     expedition.foundLoot.components.push({ item_id: itemId });
                                }
                            }
            ...
            // (Futuro: Penalidade de morte - perder dias ou loot)
            // Por enquanto, apenas perde HP (já aplicado)
        }

        updateState({ expedition: expedition });

        // 4. Abre o Modal de Resultado de Combate
        // Combate NUNCA obedece 'skipAnimations' (é um evento principal)
    ...
    endExpedition: function() {
        ...
        // 2. Salva Equipamentos e Componentes (Gerando instâncias reais se necessário)
        // (Para o MVP, assumimos que 'components' já são objetos ou IDs. Ajuste conforme necessidade)
        // Aqui, vamos apenas concatenar os arrays de objetos que criamos durante o loot
        const newComponents = expedition.foundLoot.components.map(c => ({
             instance_id: Date.now() + Math.floor(Math.random() * 10000), // Gera ID único
             item_id: c.item_id
        }));
        
        playerInventory.components = playerInventory.components.concat(newComponents);
        playerInventory.equipment = playerInventory.equipment.concat(expedition.foundLoot.equipment);

        const lootCount = Object.keys(expedition.foundLoot.materials).length + newComponents.length;
        const logMessage = lootCount > 0 ? "Expedition ended. Loot secured." : "Expedition ended. No loot found.";
